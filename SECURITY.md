...
Prereleases and stable releases use the same checks on their own source and package. Stable releases are prepared from protected `main`; they may contain reviewed runtime changes. The installed tarball, privacy leakage, archival prompt-injection boundary, client namespace, exact package contents, and clean-room setup are verified for the release being published. The workflow re-verifies exact-source security attestations immediately before npm publication. Display-state checkboxes do not authorize publication.
