...
Stable publication is fail-closed until the protected `attest-canary.yml` workflow verifies npm's signed SLSA provenance, exact public release-candidate digest, source commit, successful release workflow, and any registry `gitHead` that is present. It uses the verified provenance transparency-log integration time as the canary start, observes at least seven elapsed days, certifies every named exercise, and attests the result. Immediately before promotion, the installed tarball, privacy leakage, archival prompt-injection boundary, client namespace, exact package contents, and clean-room privacy-first setup run again. Stable source must be the direct child of the canaried RC evidence commit and may change exactly the reviewed version and verification metadata. Package, reviewed dependency lock, plugin, registry, screenshot manifest, and client metadata are checked field by field so only exact version and channel locations can change; runtime and every other packaged byte remain identical. The release workflow re-verifies that exact-sha attestation immediately before npm publication. Editable registry dates, source dates, or checkboxes never authorize stable publication by themselves.
